"""OpenAPI schema 富化层（唯一 schema 权威的来源）。

`enrich(schema)` 对 FastAPI 原生 openapi schema 做就地增强，供两处共用：
    - `app.py`：覆盖 `app.openapi`，让活的 `/docs` 展示富化谱；
    - `scripts/gen_openapi.py`：导出 `docs/openapi.json`。
两者用同一份 `enrich` + `get_version()`，从根上消除「活 /docs vs 导出 json vs 版本」三方漂移。

四层富化信息：
    - ENRICHMENTS：每个 (path, method) 的 summary / description
    - GLOBAL_PARAM_DOCS + PARAM_OVERRIDES：query/path 参数的 description / enum / example
    - SCHEMA_DOCS：component schema 的整体说明、逐属性 description、请求示例
    - 顶层 info.description / servers / tags

设计说明：所有响应包在 `ResponseWrapper{code,message,data}`，`data` 为 `Any` → FastAPI 不为响应
生成 schema。响应形态原由 operation description 散文承载；后续 RESPONSE_DATA 注入会把响应模型补进
components + responses（供 gen_doc_tables 生成响应字段表）。
"""
    ...
    "/file/parse": {
        "post": {
            ...
            "description": (
                ...
                "服务端按 `type_id` 的参数定义执行 `默认值 <- 传入值覆盖`，并把完整快照写入 "
                "`files.input_params`，因此后续 retry 沿用上传时的值。非法 JSON、顶层非对象、对象/数组值、"
                "未知 key，或缺少无默认值的必填参数，均返回 **HTTP 400**；校验发生在建档和写 PDF 之前。\n\n"
    ...
    "/analysis/run": {
        "post": {
            ...
            "description": (
                ...
                "（取 `files.type_id`，传了且不一致该 item 报错）\n"
                "- `persist=true`（仅 `source=file` 可用）把结果 upsert 进 `analysis_result`，"
                "**不改 `files.progress`**；文件不存在 / 无提取结果等查库才知道的问题记在 item 级 "
                "`error` 字段，不影响同批其它 item\n"
...
_SEARCH_CONFIG_DOC = (
    ...
    "- `vector_db`：`query_text`（单串或数组，数组=多路检索，每路一个独立占位符标签）"
    " / `top_k`（可选，不配则走阈值+相对分差）/ `score_threshold` / `score_ratio`(0.85)\n"
...
SCHEMA_DOCS: Dict[str, Dict[str, Any]] = {
    ...
    "AnalysisRunItem": {
        ...
        "properties": {
            ...
            "field_values": (
                "外部传入的 `{field_id: value}` 映射，替代文件提取结果；"
                "`source=values` 时必填，`source=file` 时禁传"
            ),
            "rule_ids": (
            ...
            "file_id": "`source=file` 时必传，取该文件已落库的 extraction_result 作字段值",
        },
    },
...
def enrich(schema: Dict[str, Any]) -> Dict[str, Any]:
    ...
    schema.setdefault("info", {})["description"] = API_INFO_DESCRIPTION
    schema["tags"] = TAGS
