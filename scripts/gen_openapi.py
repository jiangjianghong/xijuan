...
ENRICHMENTS: Dict[str, Dict[str, Dict[str, Any]]] = {
    ...
    "/doctype": {
        "post": {
            ...
            "description": (
                ...
                "**`is_default` 保留原值**（即默认类型可以改 `type_name`，但不会变成非默认；反之亦然）\n\n"
                "`type_id` 必须匹配 `^[a-zA-Z0-9_-]+$`（最长 64）。"
            ),
        }
    },
    "/doctype/{type_id}": {
    ...
    "/file/parse": {
        "post": {
            ...
            "description": (
                ...
                "**`mode` 行为差异**\n"
                "- `async`（默认）：后台任务，立即返回 `file_id`\n"
                "- `sync`：阻塞直到全部完成或失败，再返回 `file_id`\n"
                "- `stream`：返回 `text/event-stream`，事件序列见 `API_DOCUMENTATION.md` 第 9.1 节\n\n"
                "**`callback_url`** 在 `async` 与 `sync` 模式下都会被使用（管线每阶段开始、每条 "
                ...
                "**文件大小限制**：受 `mineru.max_file_size` 控制（默认 100MB），超限返回 "
                "`ResponseWrapper{code:400, message:\"文件大小超过限制 (XXX MB)\"}`。"
            ),
        }
    },
    "/file/list": {
        "get": {
            ...
            "description": (
                ...
                "**`status` 合法值**（见 `progress` 字段）：`parsing` / `tableing` / `chunking` / "
                "`embedding` / `extracting` / `analyzing` / `complete`，以及对应的 `*_failed` 失败态。\n\n"
                "返回 `FileListResponse{items, total, page, page_size, total_pages}`。"
            ),
        }
    },
    "/file/batch": {
    ...
    "/file/{file_id}/detail": {
        "get": {
            "summary": "文件完整详情",
            "description": (
                "在 `/status` 字段基础上额外返回 `start_*_time` / `end_*_time` 全套时间戳"
    ...
    "/file/{file_id}/retry/{stage}": {
        "post": {
            ...
            "description": (
                ...
                "`stream` 返回的事件序列与 `/file/parse?mode=stream` 完全一致，**多一条** "
                "首发 `resume` 事件标识从哪个阶段继续。\n\n"
    ...
    "/file/{file_id}/retry/extracting": {
        "post": {
            "summary": "快捷重试：字段提取",
            "description": (
                "语义等价于 `POST /file/{file_id}/retry/{stage}` 中 `stage=extracting`，"
                "内部就是转发调用。\n\n"
    ...
    "/file/{file_id}/retry/analyzing": {
        "post": {
            "summary": "快捷重试：逻辑分析",
            "description": (
                "语义等价于 `POST /file/{file_id}/retry/{stage}` 中 `stage=analyzing`，"
                "内部就是转发调用。\n\n"
                "支持参数：`mode`（async/sync/stream）、`callback_url`（async/sync 生效）。"
            ),
        }
    ...
    "/extraction/test": {
        "post": {
            ...
            "description": (
                ...
                "（VL 直出 JSON，不再二次抽取）\n\n"
                "**状态码**: 200 / 400（既未传 `field_id` 也未传 `config`）/ 404（字段或文件内容不存在）/ 500"
            ),
        }
    },
    ...
    "/analysis/test": {
        "post": {
            ...
            "description": (
                ...
                "- `result_value`：最终结果\n"
                "  - `judge` 类型：调用 LLM 判断（受 `system_prompt` 控制），通常返回 `true`/`false`\n"
                "  - `calc` 类型：`numexpr` 计算，按 `analysis.calc_precision`（默认 2 位）保留小数\n"
                "- `reason`：LLM 给出的判断理由（calc 类型为计算公式）\n\n"
                "**状态码**: 200 / 400（既未传 `rule_id` 也未传 `config`）/ 404（规则不存在）/ 500"
            ),
        }
    },
    ...
    "/search": {
        "post": {
            ...
            "description": (
                ...
                "**响应字段**：`chunk_id` / `file_id` / `chunk_index` / `total_chunks` / "
