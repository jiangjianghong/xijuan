...
## 增补(2026-07-14):content_list 未根治,改用全局唯一锚 + LIS

### 现象复发
用上文 content_list 方案上线后,重新上传同一份 381MB / 436 页扫描拼接件
(多份扫描支撑材料装订在项目正文之后)重解析抽取,**12 处村庄"情况说明"检索结果
页码仍全部相同**,与真实页(第 208 / 218 / 232 页)差 180 多页。

### 溯源定位(检索对,页码映射错)
落库数据诊断结论:
- 那 12 处是 12 份**真正不同**的村庄材料,检索按不同 md 位置(317k~328k)正确命中。
- content_list 重放建出的 page_mapping **只覆盖 42 个不同页、最大 page 233**,且 md
  36k~372k(占全文 77%,33 万字符)**零锚点**——检索位置落在这个黑洞里,`bisect` 把
  它们全部退化到黑洞左端最近的锚点第 41 页。
- middle_json **完整覆盖 436 页**(仅 1 页空),真实 page_idx 正确(村庄材料确在
  第 208/218/232 页)。md 顺序**严格等于**页码顺序(216 全局唯一锚 0 逆序)。

### 真因(两套算法共有)
病灶不是 content_list 特有,而是**「短前缀 + 单调游标 `md.find(prefix, cursor)` +
无唯一性校验」**:巨型 OCR 表格区里 `<table><tr><td>` 类短前缀在 md 重复上千次,
游标限定 find 匹配到错误处、把游标冲过头,此后真实位置在游标之前的块全部 miss
→ 大面积零锚点黑洞 → 页码塌缩。content_list 重放同样靠单调游标,表格探针唯一性
更差(覆盖 42 页 < 老算法 68 页),不但没根治反而更糟;14 页小样本 74/74 只是没有
大量重复表格,掩盖了通病。

### 终局方案:全局唯一锚 + LIS 单调清洗
`build_page_mapping` 重写(数据源仅 middle_json):
1. 每块取足够长前缀(文本块 span content;表格块 table_body span 的 `html`,取代
   `<table` 盲锚),在**整篇 md 做全局唯一匹配**(`count==1` 才认),得可信锚
   `(pos, page_num, bbox, page_size)`;bbox 为 middle_json 原生页坐标,无需反归一化。
2. 锚点按位置排序,用 **LIS**(最长非降子序列)剔除极少数破坏单调的假唯一匹配。
3. `lookup_page_num`/`lookup_bboxes` 用 bisect 在相邻锚点间回填,schema 不变。

真实文件效果:锚点覆盖 42 → **320 页**,0 逆序,村庄材料回到 **208/218/232**。

### 链路变更
- content_list 页码映射链路整体移除(mineru 不再请求 `return_content_list`,
  `parse_file` 回到 2 元组,删 `build_page_mapping_auto`/`build_page_mapping_from_content_list`)。
- 新增 `POST /file/{id}/recompute_page_mapping`:用落库 md+middle_json 重算刷新
  存量文件页码映射,无需重新上传/解析。
