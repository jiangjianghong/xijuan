...
> **关键词类检索（`context` / `rule` / `chunk_db`）共用的排序与截断规则**
>
> - `sort_order`：`relevance`（默认）/ `asc` / `desc`。**相关度** = 命中片段里出现的**不同**关键词的 IDF 之和（同一关键词重复出现只计一次），IDF 语料是该文件的分块。配单个关键词时所有候选同分，并列按位置升序 —— 行为与 `asc` 完全一致。
> - `max_results`：**每个关键词**最多返回几条，不是总条数。
> - `max_total_results`：总条数上限。**缺省 = `max_results`**（总量与旧版本一致，只是分配方式变成轮转公平）；显式写 `0` = 不限（上限为 `max_results × 关键词数`）；写正整数就是该值。超限时**按关键词轮流裁剪**（round-robin），只要该值 ≥ 关键词数，每个关键词的 `<search_result>` 占位符都保证有内容。
>
> ⚠️ **行为变更**：`context` / `rule` 的 `max_results` 原本是跨关键词的**全局**上限，现在改成**每关键词**上限。但因为 `max_total_results` 缺省等于 `max_results`，**存量配置的注入片段总量不变**，变的只是分配方式——原先 5 个名额可能被高频关键词全部占光（低频关键词占位符静默为空），现在按关键词轮流分配。想要更多片段，显式调大 `max_total_results` 或设成 `0`（不限）。
>
...
关键词命中点前取 `context_before`（默认 200）字符、后取 `context_after`（默认 200）字符。`max_results`（默认 5）限制**每个关键词**的条数，`max_total_results` 限制总条数。多关键词时**每个关键词一个占位符**，默认按相关度排序（见上方共用规则）。
...
找到关键词后按 `direction` 扩展（`forward`=向关键词**之后**扩展，`backward`=向关键词**之前**扩展，`both`=双向；默认 forward），止于最近的停用词，切出一段。默认停用词 `["#", "##", "###", "\n\n", "\n", "。", ".", "；", ";"]`；`min_length`（2）、`max_length`（200）兜底。适合「关键词 + 一小段值」的精确提取。`max_results`（默认 5）限制**每个关键词**的条数，`max_total_results` 限制总条数；默认按相关度排序，打分作用在切出的片段（`extracted_text`）上而非整个窗口（见上方共用规则）。
...
从入库的文本分块里按 `keywords` 过滤，`max_results`（默认 10，亦可写 `top_k`）限制**每个关键词**的分块数，`max_total_results` 限制总条数。适合「相关内容散落多段、要整段喂 LLM 归纳」的场景。默认按相关度排序（见上方共用规则）。
