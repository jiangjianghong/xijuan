...
const SettingsManager = {
    ...
    FIELDS: {
        ...
        embedding: [
            ['base_url', '服务地址', 'url'], ['model_name', '模型名称', 'text'],
        ...
        extraction: [
            ['llm_base_url', '服务地址', 'url'], ['llm_model', '模型名称', 'text'],
            ['llm_api_key', 'API Key', 'secret'],
            ['llm_timeout', '请求超时', 'number', { min: 1, unit: '秒' }],
            ['llm_retry_count', '重试次数', 'number', { min: 1 }],
            ['max_context_length', '最大上下文长度', 'number', { min: 1, unit: '字符' }],
            ['llm_extra_body', '额外请求参数', 'json', { rows: 4 }],
        ],
        table_name_validation: [
            ['llm_base_url', '服务地址', 'url', { nullable: true }],
            ['llm_model', '模型名称', 'text', { nullable: true }],
            ['llm_api_key', 'API Key', 'secret'],
            ['llm_timeout', '请求超时', 'number', { min: 1, unit: '秒', nullable: true }],
            ['llm_retry_count', '重试次数', 'number', { min: 1, nullable: true }],
            ['max_context_length', '最大上下文长度', 'number', { min: 1, unit: '字符', nullable: true }],
            ['max_context_lines', '参考行数', 'number', { min: 1, nullable: true }],
            ['max_concurrency', '最大并发', 'number', { min: 1, nullable: true }],
            ['llm_extra_body', '额外请求参数', 'json', { rows: 4, nullable: true }],
        ],
        analysis: [
            ['calc_precision', '计算精度', 'number', { min: 0, unit: '位小数' }],
            ['judge_timeout', '判定超时', 'number', { min: 1, unit: '秒' }],
            ['max_concurrency', '独立分析并发', 'number', { min: 1 }],
        ],
        vl_model: [
            ...
            ['global_max_concurrency', '全局最大并发', 'number', { min: 1 }],
